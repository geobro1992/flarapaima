...
for (s in 1:n.sites){  
  ...
  for(t in 3:n.years){
    ...
    HA[s,t-1] ~ dbin(h, SA[s,t-1]) 
    
    N.A[s,t] <- max((SA[s,t-1] + M[s,t-1] - HA[s,t-1]),1)
...
cat(file="model_ih_uninformed.txt", " 
    model {
    #priors
    ...
    n.ih ~ dpois(mu.ih)             # illegal fishing
    mu.ih ~ dunif(0, 100) 
...
for (s in 1:n.sites){  
  ...
  for(t in 3:n.years){
    ...
    N.A[s,t] <- max((SA[s,t-1] + M[s,t-1] -HA[s,t-1] - n.ih),1)
    N.J[s,t] <- max((SJ[s,t-1] + R[s,t-2] - M[s,t-1]),1)
    } # t
} #s
...
cat(file="model_ih_informed.txt", " 
    model {
    #priors
    ...
    n.ih ~ dpois(mu.ih)              # illegal fishing (informed)
    mu.ih ~ dunif(0, 7) 
