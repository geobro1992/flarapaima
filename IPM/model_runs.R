...
for(s in 1:n.sites){
  ...
  for(t in 3:n.years){
    ...
    H[s,t-1] = min(as.integer((SA[s,t-1] * h) + ih), as.integer((SA[s,t-1] + M[s,t-1])))
...
C = H - ih                                         # observed catch with no unreported fishing
C[which(C < 0)] = 0
...
#call JAGS from R
out1= jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "model_no_ih.txt", 
           n.iter=ni, n.burnin = nb, n.chains= nc,
           n.thin= nt, n.adapt = na, parallel = TRUE)

save(out1, file = "no_ih.RData")
...
#call JAGS from R
out2= jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "model_ih_uninformed.txt", 
           n.iter=ni, n.burnin = nb, n.chains= nc,
           n.thin= nt, n.adapt = na, parallel = TRUE)

save(out2, file = "ih_uninformed.RData")
...
#call JAGS from R
out3= jags(data = jags.data, inits = inits, parameters.to.save = parameters, model.file = "model_ih_informed.txt", 
           n.iter=ni, n.burnin = nb, n.chains= nc,
           n.thin= nt, n.adapt = na, parallel = TRUE)

save(out3, file = "ih_informed.RData")
